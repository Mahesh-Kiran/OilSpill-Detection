...
# ---------- main: config, model load, and batch loop ----------
class Args:
    img_size = 224
    num_classes = 2
    vit_patches_size = 16
    n_skip = 3
    model_weights = r'C:\Users\Mahesh Kiran\Downloads\final_model.pth'  # change if needed
...
config.patches.grid = (args.img_size // args.vit_patches_size, args.img_size // args.vit_patches_size)

device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')

model = VisionTransformer(config, img_size=args.img_size, num_classes=args.num_classes).to(device)

# Try loading state dict. If strict load fails, try non-strict to allow minor mismatches.
try:
    sd = torch.load(args.model_weights, map_location=device)
    try:
        ...
        try:
            ...
            print("Failed to load state_dict with both strict and non-strict. Raising original error.")
            raise e2
except Exception as e:
    print("Error loading model weights:", e)
    raise

transform = ResizeToTensor(size=(args.img_size, args.img_size))

# === Set these paths ===
# If input_path is a folder, all supported images inside will be processed.
# If input_path is a single image file, only that file will be processed.
input_path = r"C:\Users\Mahesh Kiran\KMIT College\Trinetra\output\output_folder_files\11"    # <-- folder or single file
output_folder = r"predicted_oil"  # <-- where masks will be saved
...
def process_file(in_file):
    ...
    out_file = os.path.join(output_folder, f"{name}_mask.png")
    try:
        predict_single_image(in_file, out_file, model, transform, device)
    except Exception as ex:
        print(f"Failed to process {in_file}: {ex}")

# Main dispatch
if os.path.isfile(input_path):
    print("Input is a single file. Processing:", input_path)
    process_file(input_path)
elif os.path.isdir(input_path):
    files = sorted(os.listdir(input_path))
    image_files = [os.path.join(input_path, f) for f in files if f.lower().endswith(valid_extensions)]
    if not image_files:
        print("No supported image files found in:", input_path)
    else:
        print(f"Found {len(image_files)} images. Processing...")
        for idx, img_file in enumerate(image_files, start=1):
            print(f"[{idx}/{len(image_files)}] {os.path.basename(img_file)}")
            process_file(img_file)
        print("All done.")
else:
    print("input_path does not exist. Please set a valid file or folder path.")
